# Generated by Django 4.1.4 on 2022-12-30 07:25

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = []

    operations = [
        migrations.CreateModel(
            name="TbName",
            fields=[
                (
                    "code",
                    models.CharField(
                        db_column="code",
                        max_length=16,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                ("name", models.CharField(db_column="name", max_length=16)),
            ],
            options={"db_table": "tb_name",},
        ),
        migrations.CreateModel(
            name="TbSentimental",
            fields=[
                (
                    "id",
                    models.AutoField(db_column="ID", primary_key=True, serialize=False),
                ),
                ("news", models.JSONField(blank=True, db_column="news", null=True)),
                ("report", models.JSONField(blank=True, db_column="report", null=True)),
                (
                    "comment",
                    models.JSONField(blank=True, db_column="comment", null=True),
                ),
                (
                    "code",
                    models.ForeignKey(
                        db_column="code",
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="summary.tbname",
                    ),
                ),
            ],
            options={"db_table": "tb_senti",},
        ),
        migrations.CreateModel(
            name="TbReport",
            fields=[
                (
                    "id",
                    models.AutoField(db_column="ID", primary_key=True, serialize=False),
                ),
                (
                    "title",
                    models.CharField(
                        blank=True, db_column="title", max_length=255, null=True
                    ),
                ),
                (
                    "content",
                    models.TextField(blank=True, db_column="content", null=True),
                ),
                ("date", models.DateTimeField(blank=True, db_column="date", null=True)),
                (
                    "firm",
                    models.CharField(
                        blank=True, db_column="firm", max_length=16, null=True
                    ),
                ),
                (
                    "comment",
                    models.CharField(
                        blank=True, db_column="comment", max_length=8, null=True
                    ),
                ),
                (
                    "price",
                    models.IntegerField(blank=True, db_column="price", null=True),
                ),
                (
                    "code",
                    models.ForeignKey(
                        db_column="code",
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="summary.tbname",
                    ),
                ),
            ],
            options={"db_table": "tb_report",},
        ),
        migrations.CreateModel(
            name="TbKeyword",
            fields=[
                (
                    "id",
                    models.AutoField(db_column="ID", primary_key=True, serialize=False),
                ),
                ("date", models.DateTimeField(blank=True, db_column="date", null=True)),
                (
                    "keyword",
                    models.JSONField(blank=True, db_column="keyword", null=True),
                ),
                (
                    "code",
                    models.ForeignKey(
                        db_column="code",
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="summary.tbname",
                    ),
                ),
            ],
            options={"db_table": "tb_keyword",},
        ),
        migrations.CreateModel(
            name="TbInfo",
            fields=[
                (
                    "id",
                    models.AutoField(db_column="ID", primary_key=True, serialize=False),
                ),
                (
                    "established_date",
                    models.DateTimeField(
                        blank=True, db_column="established_date", null=True
                    ),
                ),
                (
                    "listed_date",
                    models.DateTimeField(
                        blank=True, db_column="listed_date", null=True
                    ),
                ),
                (
                    "CEO",
                    models.CharField(
                        blank=True, db_column="CEO", max_length=8, null=True
                    ),
                ),
                (
                    "settlement_date",
                    models.DateTimeField(
                        blank=True, db_column="settlement_date", null=True
                    ),
                ),
                (
                    "tel",
                    models.CharField(
                        blank=True, db_column="tel", max_length=16, null=True
                    ),
                ),
                (
                    "code",
                    models.ForeignKey(
                        db_column="code",
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="summary.tbname",
                    ),
                ),
            ],
            options={"db_table": "tb_info",},
        ),
    ]
